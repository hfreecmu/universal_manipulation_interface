...
with SharedMemoryManager() as shm_manager:
    ...
    # just use -1 even though it has timesteps 
    # just for now
    # ee_im = cv2.cvtColor(camera_data['color'][-1], cv2.COLOR_BGR2RGB)
    # if ee_im.shape[0] > ee_im.shape[1]:
    #     trim = (ee_im.shape[0] - ee_im.shape[1])//2
    #     ee_im = ee_im[trim:-trim]
    # elif ee_im.shape[1] > ee_im.shape[0]:
    #     trim = (ee_im.shape[1] - ee_im.shape[0])//2
    #     ee_im = ee_im[:, trim:-trim]

    # ee_im_resized = cv2.resize(ee_im, (256, 256))
    # obs_image = ee_im_resized
